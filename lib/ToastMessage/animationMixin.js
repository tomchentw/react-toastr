import { default as ReactTransitionEvents } from "react/lib/ReactTransitionEvents";

import { default as ReactDOM } from "react-dom";

import { default as elementClass } from "element-class";
...
export default {
  ...
  _set_transition: function _set_transition(hide) {
    ...
    var endListener = function endListener(e) {
      ...
      var classList = elementClass(node);
  ...
  _set_animation: function _set_animation(hide) {
    ...
    var endListener = function endListener(e) {
      ...
      animations.forEach(function (anim) {
        return elementClass(node).remove(anim);
      });
    ...
    animations.forEach(function (anim) {
      return elementClass(node).add(anim);
    });
  },
  ...
  _clear_animation: function _clear_animation(hide) {
    ...
    animations.forEach(function (animation) {
      return elementClass(node).remove(animation);
    });
  },
  ...
  _flush_class_name_queue: function _flush_class_name_queue() {
    if (this._is_mounted) {
      ...
      this.classNameQueue.forEach(function (className) {
        return elementClass(node).add(className);
      });
    }
